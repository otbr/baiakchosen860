...
function creatureSayCallback(cid, type, msg)
	...
	selfSay('Olá ' .. getCreatureName(cid) .. ', fale {addon 1} para mais informações!',cid)

	if(msgcontains(msg, 'addon 1') or msgcontains(msg, 'first addon') or msgcontains(msg, 'addon')) then
		selfSay('Para conseguir o addon 1 do Warmaster eu preciso que você me traga uma {Zaoan Sword}, você trouxe uma?', cid)
	...
	elseif(msgcontains(msg, 'yes') and talkState[talkUser] == 1) then
		if(getPlayerItemCount(cid, 11301) >= 1) then
			...
			selfSay('Desculpe, você não tem uma {Zaoan Sword}.', cid)
		end
